...
{
    if(m_curCF != pfft->GetCenterFreq() || m_curBW != pfft->GetBWHz())  //for detecting changes in the cf or bw
    {
        //reset detection map
        m_curCF = pfft->GetCenterFreq();
...
void SignalDetector::Update()
{
    ...
    int cntridx;
    float cntrfreqHz;
    //float averagenoisefloor,
         //   float avstddev;;
    //averagenoisefloor = m_peakdetector->AverageNoiseFloor(&avstddev);
    ...
    for(QMap<int,float>::iterator it = m_peakdetector->m_Peaks.begin(); it != m_peakdetector->m_Peaks.end(); ++it)
    {
        ...
        float *tdat = pfft->AvgData();

        float rbw = pfft->GetRBWHz();// get resolution bandwidth
        float sigwidth = 12500;
        float floorwidth = (sigwidth * SIG_WIDTH_SCALER) / rbw;
        if(floorwidth < 10 )
            floorwidth = 10;

        float localnoisefloor = m_peakdetector->LocalNoiseFloor(cntridx,(int)floorwidth,&stddev,tdat);
        sd->localnoisefloor = localnoisefloor;
        ...
        //float dbmrange_av = centerdbm - averagenoisefloor;
        //float lowthreshold_av = ((dbmrange_av / 3) + averagenoisefloor); // not used

        float dbmrange = centerdbm - localnoisefloor;
        float lowthreshold = ((dbmrange / 3) + localnoisefloor);
...
void SignalDetector::InitMap()
{
    ...
    //clear old data
    for(int c =0; c < MAX_FFT_SIZE; c++)m_map[c] = 0;
